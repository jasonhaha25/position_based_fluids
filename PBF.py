...
class PBF:
    def __init__(self, dim):
        ...
        # Neighbour logic
        self.particles_in_cell = ti.field(ti.i32, shape=(gridX, gridY, num_max_particles_in_cell))
        self.num_particles_in_cell = ti.field(ti.i32, shape=(gridX, gridY))
        ...
        # visual
        self.boundary = ti.Vector.field(2, ti.f32, shape=1)
        for i in range(1):
    ...
    def particle_init(self):
        # initial particle starting positions
        start = (20, 10)
        offsetX = 1
        offsetY = 1
        for i in range(40):
            for j in range(25):
                self.positions[i * 25 + j] = ti.Vector([start[0] + i * offsetX, start[1] + j * offsetY])
    ...
    def calculate_lambda(self):
        for i in range(num_particles):
            ...
            for j_index in range(self.num_neighbours[i]):
                ...
                # Eq(7)
                grad_pj_Ci = Util.W_spiky_gradient(pos_ij)
    ...
    def calculate_delta_p(self):
        ...
        for i in range(num_particles):
            ...
            self.delta_p[i] = ti.Vector([0.0, 0.0])
            for j_index in range(self.num_neighbours[i]):
                j = self.neighbours[i, j_index]
                pos_j = self.positions[j]
                lambda_j = self.lambdas[j]
                pos_ij = pos_i - pos_j
                self.delta_p[i] += (lambda_i + lambda_j + Util.s_corr(pos_ij)) * Util.W_spiky_gradient(pos_ij)
    ...
    def run(self):
        ...
        while self.gui.is_running() and self.gui.not_escape():
            if frame % 50 == 0:
                self.b_amount *= -1
            ...
            self.algo1()
            if self.dim == 2:
                self.gui.render(self.positions, self.neighbours, self.boundary)
